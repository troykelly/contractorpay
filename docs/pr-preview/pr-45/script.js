...
(function () {
  ...
  function calcRepaymentIncome(taxable, superRate) {
    ...
    return taxable + net + fringe + included + additional + exempt;
  }

  function updateRepaymentIncome(taxable, fy, superRate, setRate = true) {
    if (!hasHecsCheckbox || !hasHecsCheckbox.checked) return 0;
    const total = calcRepaymentIncome(taxable, superRate);
    if (setRate) {
  ...
  function updateRateChangeUI() {
    ...
    const ri = calcRepaymentIncome(
      daily * baseWorkingDays,
  ...
  async function calculate() {
    ...
    const taxableForRate = fteSalary > 0 ? fteSalary : periodIncome;
    let fteRi = 0;
    if (hasHecsCheckbox && hasHecsCheckbox.checked) {
      if (fteSalary > 0) {
        fteRi = calcRepaymentIncome(fteSalary, superRate);
      }
      updateRepaymentIncome(taxableForRate, fy, superRate);
    ...
    const periodRi = calcRepaymentIncome(periodIncome, superRate);
  ...
  async function createLedger() {
    ...
    const totalsRi = calcRepaymentIncome(
      dailyRate * baseWorkingDays,
      superRate,
    );
    ...
    while (currentStart <= endDate) {
      ...
      const ri = calcRepaymentIncome(dailyRate * workingDays, superRate);
